...
plt.plot(range(len(Y_train)), Y_train, label="Эталон (обучение)")
plt.plot(range(len(Y_train)), train_pred, label="Прогноз")
plt.title(f"График прогнозируемой функции на участке обучения")
plt.legend()
plt.grid(True)
plt.show()
...
plt.plot(losses)
plt.title(f"График изменения ошибки в зависимости от итерации")
plt.xlabel("Эпоха")
plt.ylabel("MSE")
plt.grid(True)
plt.show()
